...
<body>
    ...
    <main>
        <div class="cuerpo-oficio mt-4">
            ...
            <div class="mt-2 text-center f15"><span class="text-gros">ASIGNATURA:</span> {{mb_strtoupper($curso->nombre_asignatura)}}  </div>
            <div class="mt-2 text-center f15"><span class="text-gros">ESTUDIANTE:</span> {{mb_strtoupper($estudiantes->primer_nombre)}} {{mb_strtoupper($estudiantes->segundo_nombre)}} {{mb_strtoupper($estudiantes->apellido_paterno)}} {{mb_strtoupper($estudiantes->apellido_materno)}} </div>
            <div class="mt-2 f14" style="height: 70px;">
                <div class="d-flex">
                    <div class="text-center" style="width:32%;"><span class="text-gros">GRADO/CURSO:</span> {{mb_strtoupper($curso->curso_nombre)}} {{mb_strtoupper($curso->curso_nivel)}}"{{mb_strtoupper($curso->curso_paralelo)}}"</div>
                ...
                <table class="table table-bordered">
                    <thead class="thead-dark-1">
                        <tr>
                            ...
                            <th class="text-center" width="50%">Calificación</th>
                        </tr>
                    </thead>
                    <tbody>
                        @if (sizeof($actividades) > 0)
                            @foreach ($actividades as $actividad)
                                <tr>
                                    <td>
                                        {{$actividad->actividad_abr}} {{$actividad->nombre}}
                                    </td>
                                    @php $contador = 0; $nota_actividad = 0.00; @endphp
                                    @foreach ($calificaciones as $notas)
                                        @php
                                            if ($notas->actividad_fk == $actividad->id) {
                                                $contador++; $nota_actividad = $notas->calificacion;
                                                break;
                                            }
                                        @endphp
                                    @endforeach
                            ...
                            @endforeach
                            <tr>
                                <td class="text-left">Promedio parcial</td>
                                <td class="text-center">
                                    @php
                                        $nota_parcial = $calificacion_parcial == null ? 0.00 : $calificacion_parcial->calificacion;
                                    @endphp
                                    {{$nota_parcial}}
                                </td>
                            </tr>
                        @else
                        <tr>
                            <td class="text-left" colspan="2">No se encontraron actividades.</td>
                        </tr>
                        @endif
                    </tbody>
                </table>
