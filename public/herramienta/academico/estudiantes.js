...
document.querySelector("#btnCreateRegister").addEventListener("click", () => {
    ...
    if ( n1 == "success" &&   n2 == "success" &&   n3 == "success" && n6 == "success"){        
        ...
        fetch("/usuarios/crear/nuevo/estudiante/fetch", {method: "POST", headers: { "X-CSRF-TOKEN": csrf.value }, body: formData })
...
document.querySelector("#btnEditRegister").addEventListener("click", () => {
    ...
    if (n1 == "success" && n2 == "success" && n3 == "success" && n6 == "success" ){
        ...
        fetch("/usuarios/editar/estudiante/fetch", {method: "POST",  headers: { "X-CSRF-TOKEN": csrf.value },  body: formData })
...
document.querySelector("#btnBanRegister").addEventListener("click", () => {
    ...
    fetch("/usuarios/banear/estudiante/fetch", { method: "POST", headers: { "X-CSRF-TOKEN": csrf.value }, body: formData})
    .then((response) => response.json()).then((data) => {
        console.log(data)
        if (data) {
            if (data.success) {
...
document.querySelector("#btnActiveRegister").addEventListener("click", () => {
    ...
    fetch("/usuarios/activar/estudiante/fetch", { method: "POST", headers: { "X-CSRF-TOKEN": csrf.value }, body: formData})
    .then((response) => response.json()).then((data) => {
        if (data) {
            if (data.success) {
                $("#contenido_datos").load(location.href+" #contenido_datos>*","");
...
document.querySelector("#btnDeleteRegister").addEventListener("click", () => {
    ...
    fetch("/usuarios/eliminar/estudiante/fetch", { method: "POST", headers: { "X-CSRF-TOKEN": csrf.value }, body: formData})
    .then((response) => response.json()).then((data) => {
        console.log(data)
        if (data) {
            if (data.success){
